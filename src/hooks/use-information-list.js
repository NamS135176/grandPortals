...
export const useInformationList = () => {
    ...
    const getListInformationSendList = useCallback(
    ...
    const getListInformationSendByInformationId = useCallback(
        async (list = [], nextToken = null, id) => {
            ...
            var informationListSend =
                response.data.queryInformationListSendByInformationId.items;
            ...
            if (
                ...
                await getListInformationSendByInformationId(
                    list,
                    response.data.queryInformationListSendByInformationId
                        .nextToken,
                    id
                );
            }
    ...
    const filterInformationSendList = useCallback((filter) => {
        if (filter == "全て") {
    ...
    let filterDateInformationSendList = (start, end, filter) => {
        ...
        if (filter == "全て") {
            const newList = informationListFirst.filter((item) => {
                if(start && end){
                    return (
                        moment(item.scheduled_delivery_date).valueOf() <
                            end.getTime() &&
                        moment(item.scheduled_delivery_date).valueOf() >
                            start.getTime()
                    );
            ...
            const newList = informationListFirst.filter((item) => {
                if(start && end){
                    ...
                    return (
                        moment(item.scheduled_delivery_date).valueOf() <
                            end.getTime() && item.status == filter
                    );
                }
                else if(start && !end){
                    return (
        ...
        async (item) => {
            try {
                ...
                if (info.data?.getInformation?.delete_flag == 0) {
                    ...
                    const newInformationList = R.reject(
                        R.propEq("id", item.id)
                    )(informationList);
                    setInformationList(newInformationList);
                    const newInformationListFirst = R.reject(
                        R.propEq("id", item.id)
    ...
    const updateReadInformation = useCallback(async (id) => {
        try {
            await API.graphql({
                query: mutations.updateInformationListSend,
                variables: {
        ...
        async function loadData() {
            ...
            if (user.group === UserGroup.agentGrands) {
                ...
                informationList = list.filter((item) => {
                    ...
                    return (
                        item.information.processed_date &&
                        item.information.draft_flag == 0 &&
                        item.information.delete_flag == 0
                    );
                });
            } else {
